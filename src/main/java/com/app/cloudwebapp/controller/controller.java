...
public class controller {
	...
	public ResponseEntity accountVerification(@RequestParam(name = "email") String email,
		...
		if (!user.get().getActive()) {
			...
			if (account != null) {
				...
				userRepository.updateUser(updateUser.getUsername(), updateUser.getFirst_name(),
						updateUser.getLast_name(), updateUser.getPassword(),
						(Timestamp) updateUser.getAccount_updated(), isActive);
				...
				return new ResponseEntity<String>("Your account is not verified!!, check token is expired or not",
						HttpStatus.OK);
		...
		if (user != null) {
			...
			if (user.get().getActive()) {
				...
				userRepository.updateUser(authentication.getName(), updatedUser.getFirst_name(),
						updatedUser.getLast_name(), updatedUser.getPassword(),
						(Timestamp) updatedUser.getAccount_updated(), updatedUser.getActive());
				logger.info("update user completed");
		...
		if (user != null) {
			...
			if (user.get().getActive()) {
				...
				String jsonString = new JSONObject().put("file_name", profilePic.get().getFile_name())
						.put("id", profilePic.get().getId()).put("url", profilePic.get().getUrl())
	...
	public ResponseEntity<String> uploadUserPic(@RequestParam("file") MultipartFile file, Authentication authentication)
		...
		if (user != null) {
			...
			if (user.get().getActive()) {
				...
				try {
					...
					// Optional<ProfilePic> picProfilePic =
					// userPicRepository.findByUser(user.get());
					...
					if (file.getContentType().equals("image/jpg") || file.getContentType().equals("image/jpeg")
						...
						ProfilePic profilePic = userPicService.uploadPicture(file, user.get().getUsername(),
								user.get());
						...
						String jsonString = new JSONObject().put("file_name", profilePic.getFile_name())
								.put("id", profilePic.getId()).put("url", profilePic.getUrl())
